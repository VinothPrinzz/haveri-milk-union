...
import {
  LayoutDashboard, ShoppingCart, Plus, XCircle,
  Package, Tag, TrendingUp,
  Factory, RefreshCw, AlertTriangle,
  MapPin, Truck, GitBranch, FileText, Clock, Globe,
  Users, UserPlus, CheckCircle, BookOpen,
  CreditCard, Receipt, DollarSign, Wallet, Landmark, Building, AlertCircle,
  BarChart3, PieChart, Map, ClipboardList, Activity,
  Settings, Bell, Megaphone, Image, Shield, UserCog, Globe2,
  ChevronDown,
} from "lucide-react";
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Stock Alerts", href: "/inventory/alerts", icon: AlertTriangle },
    ],
  },
  {
    ...
    items: [
      ...
      { name: "Vehicle Master", href: "/distribution/vehicles", icon: Truck },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Bank Reconciliation", href: "/finance/bank-recon", icon: Building },
  ...
  },
  {
    label: "SYSTEM",
    items: [
      { name: "General", href: "/system/settings", icon: Settings },
  ...
  return (
    <div className="mb-0.5">
      <button
        ...
        className="flex w-full items-center justify-between px-3 py-1.5 text-[10px] font-semibold text-muted-fg uppercase tracking-wider hover:text-fg transition-colors"
      >
        {label}
        <ChevronDown
          className={cn(
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-2.5 px-3 py-[7px] mx-1.5 rounded-lg text-[11px] font-semibold transition-colors",
                  active
    ...
    <aside className="w-[230px] bg-[#FAFAFA] border-r border-border flex flex-col shrink-0 h-screen sticky top-0 overflow-y-auto">
      ...
      <div className="px-4 py-3.5 border-b border-border">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-white text-sm">
            🐄
          </div>
          <div>
            <div className="font-display text-[10px] font-bold text-fg leading-tight">
              Haveri Milk Union
            </div>
            <div className="text-[8px] text-muted-fg font-medium">
              ERP System
            </div>
          </div>
        </div>
        {user && (
          <div className="mt-2.5 bg-muted border border-border rounded-md px-2.5 py-1.5 text-[10px] font-semibold text-muted-fg flex items-center justify-between">
      ...
      <nav className="flex-1 py-2 overflow-y-auto">
        ...
        <Link
          href="/dashboard"
          className={cn(
            "flex items-center gap-2.5 px-4 py-[7px] mx-1.5 rounded-lg text-[11px] font-semibold mb-1 transition-colors",
            pathname === "/dashboard"
              ? "bg-brand-light text-brand"
              : "text-muted-fg hover:bg-muted hover:text-fg"
          )}
      ...
      <div className="px-4 py-3 border-t border-border">
        <p className="text-[9px] text-muted-fg font-medium">
