...
export default function ProductListPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => api.get("/api/v1/products/all", { page: 1, limit: 50 }),
  });
  ...
  return (
    <>
      <PageHeader
        icon="📦"
        title="Product List"
        subtitle="Manage all dairy products and their details"
        actions={
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 bg-card border border-border rounded-lg px-3 py-2 max-w-md">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search products..." className="bg-transparent text-[12px] text-fg placeholder-muted-fg outline-none w-full font-medium" />
        </div>
      </div>
      ...
      <TableCard>
        ...
        <tbody>
          {products.map((p: any) => {
            ...
            const final_ = base * (1 + gst / 100);
            return (
              <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                <Td className="font-semibold"><span className="mr-1.5">{p.icon || "📦"}</span>{p.name}</Td>
                <Td>{p.categoryName}</Td>
                <Td>{p.unit}</Td>
                <Td className="text-right">{formatCurrency(base)}</Td>
                <Td className="text-right">{gst}%</Td>
                <Td className="text-right font-bold text-brand">{formatCurrency(final_)}</Td>
                <Td className={`text-right font-semibold ${p.stock === 0 ? "text-danger" : p.stock < 50 ? "text-warning" : ""}`}>{p.stock}</Td>
                <Td><Badge variant={p.available ? "active" : "inactive"}>{p.available ? "Active" : "Inactive"}</Badge></Td>
                <Td>
