...
export default function DealerLedgerPage() {
  const { data: dealersData } = useQuery({ queryKey: ["dealers-ledger"], queryFn: () => api.get("/api/v1/dealers", { page: 1, limit: 20 }) });
  ...
  const { data: ledgerData } = useQuery({
    ...
    queryFn: () => sel ? api.get(`/api/v1/dealers/${sel}/ledger`, { page: 1, limit: 30 }) : null,
    enabled: !!sel,
  });
      ...
      <TableCard>
        ...
        <tbody>{entries.map((e: any) => (
          <tr key={e.id} className="hover:bg-muted/50">
            ...
            <Td className="font-mono text-[10px]">{e.referenceType}</Td><Td>{e.description||"—"}</Td>
        ...
        ))}{entries.length===0&&<tr><Td colSpan={6} className="text-center py-8 text-muted-fg">No transactions</Td></tr>}</tbody>
      </TableCard>
    </>
  );
}
