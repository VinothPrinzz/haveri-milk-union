// apps/web/src/pages/sales-reports/milkCurdSalesShared.tsx
// ════════════════════════════════════════════════════════════════════
// Shared "MILK & CURD SALES REPORT" cross-tab — route × product, split
// into Night / Afternoon sales groups (by the route's batch), with a
// previous-period total-milk comparison + diff, per-group sub-totals and
// a grand "HVR TOTAL".
//
// Used by both the Daily Sales Report (single day vs previous day) and the
// Monthly Sales Report (one month vs previous month). The only thing that
// differs between them is the two comparison-column headers and the report
// title — everything else (layout, totals, CSV + Excel export) is identical
// and lives here.
//
// Print: A4 landscape (split Milk / Curd pages). Export: CSV + Excel
// (.xlsx, merged group headers) via exceljs (dynamically imported).
// ════════════════════════════════════════════════════════════════════
...
// The Daily and Monthly responses share these three fields — that is all the
// cross-tab renderer needs, so both response shapes satisfy this structurally.
export interface MilkCurdSalesData {
...
export const MILK_SLOTS = SLOTS.slice(0, MILK_SPLIT);
export const CURD_SLOTS = SLOTS.slice(MILK_SPLIT);
...
// ── Formatters ──────────────────────────────────────────────────────
export const fmtDDMM = (iso: string) => {
  if (!iso) return "";
...
// ════════════════════════════════════════════════════════════════════
export function MilkCurdSalesTable({
  data,
  slots,
  prevHeader,
  curHeader,
}: {
  data: MilkCurdSalesData;
  slots: Slot[];
  prevHeader: string;
  curHeader: string;
}) {
  ...
  return (
    ...
    >
      ...
      <colgroup>
        ...
        <col style={{ width: "4.5%" }} />   {/* prev period */}
        <col style={{ width: "4.5%" }} />   {/* selected period */}
      ...
      <thead>
        ...
        <tr className="bg-sky-100">
          {FIXED_HEADERS(prevHeader, curHeader).map((h, i) => (
...
// ── Build the two print pages (Milk items, then Curd items) ──
export const buildMilkCurdPages = (
  data: MilkCurdSalesData,
  prevHeader: string,
  curHeader: string,
): ReactNode[] => [
  <MilkCurdSalesTable key="milk" data={data} slots={MILK_SLOTS} prevHeader={prevHeader} curHeader={curHeader} />,
  <MilkCurdSalesTable key="curd" data={data} slots={CURD_SLOTS} prevHeader={prevHeader} curHeader={curHeader} />,
];

// ════════════════════════════════════════════════════════════════════
// CSV — single header row + all data/sub-total/total rows
// ════════════════════════════════════════════════════════════════════
export function buildMilkCurdCsv(
  data: MilkCurdSalesData,
  opts: { title: string; prevHeader: string; curHeader: string },
): string {
  return toCsv(buildMilkCurdCsvRows(data, opts));
}

function buildMilkCurdCsvRows(
  data: MilkCurdSalesData,
  opts: { title: string; prevHeader: string; curHeader: string },
): (string | number)[][] {
  ...
  out.push([opts.title]);
  out.push([
    "SL", "Route Name", opts.prevHeader, opts.curHeader, "Diff",
...
// ════════════════════════════════════════════════════════════════════
export async function buildMilkCurdXlsx(
  data: MilkCurdSalesData,
  opts: { sheetName: string; title: string; prevHeader: string; curHeader: string },
): Promise<Blob> {
  ...
  // Header rows (2 & 3)
  FIXED_HEADERS(opts.prevHeader, opts.curHeader).forEach((h, i) => {
