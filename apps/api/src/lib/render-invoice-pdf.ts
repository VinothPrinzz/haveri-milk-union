// ════════════════════════════════════════════════════════════════════
// apps/api/src/lib/render-invoice-pdf.ts
//
// Shared PDF renderer for HMU dealer tax invoices.
// Uses pdf-lib with built-in standard fonts (no external font files).
//
...
export async function renderInvoicePdf(p: InvoiceRenderParams): Promise<Uint8Array> {
  ...
  const ROW_H            = 14;
  const HDR_H            = 16;
  const BOTTOM_THRESHOLD = MARGIN + 80;            // reserve room for totals
  ...
  // Conditional spread avoids passing `undefined` to exactOptionalPropertyTypes fields.
  function fillRect(
    ...
    page.drawRectangle({
      ...
      ...(border !== undefined ? { borderColor: border, borderWidth: 0.5 }   : { borderWidth: 0 }),
    });
  }
  ...
  // TAX INVOICE badge (white box on right)
  const BADGE_W = 108;
  ...
  drawText(p.invoiceNumber,   MARGIN + LBLW, ly, { size: 8 });
  ly -= 13;
  drawText("Invoice Date :",  MARGIN + 6, ly, { size: 8, bold: true });
  drawText(fmtDate(p.issueDate), MARGIN + LBLW, ly, { size: 8 });
  ly -= 13;
  drawText("Delivery Date :", MARGIN + 6, ly, { size: 8, bold: true });
  drawText(fmtDate(p.deliveryDate), MARGIN + LBLW, ly, { size: 8 });
  ly -= 13;
  drawText("Order ID :",      MARGIN + 6, ly, { size: 8, bold: true });
  drawText(p.orderId.slice(0, 16).toUpperCase(), MARGIN + LBLW, ly, { size: 8 });
  ly -= 13;
  drawText("Payment :",       MARGIN + 6, ly, { size: 8, bold: true });
  const payLabel = p.paymentMode.charAt(0).toUpperCase() + p.paymentMode.slice(1);
  drawText(payLabel, MARGIN + LBLW, ly, { size: 8 });
  if (p.paid) {
    const lw = fontReg.widthOfTextAtSize(payLabel, 8);
    drawText("PAID", MARGIN + LBLW + lw + 10, ly, { size: 8, bold: true, color: C_GREEN });
  }
  ...
  const addrParts = [p.dealer.address, p.dealer.city, p.dealer.state, p.dealer.pin].filter(
    (s): s is string => s != null && s.length > 0
  );
  if (addrParts.length > 0) {
    drawText(addrParts.join(", ").slice(0, 52), COL_R + 6, ry, { size: 7.5, color: C_GRAY });
  }

  y -= BOX_H + 6;

  // Route strip
  const routeParts = [p.route.code, p.route.name].filter((s): s is string => s != null && s.length > 0);
  if (routeParts.length > 0) {
    drawText(`Route: ${routeParts.join(" - ")}`, MARGIN, y, { size: 8, color: C_GRAY });
    y -= 14;
  } else {
    y -= 4;
  }

  // ── Table header (reused on continuation pages) ──────────────────
  ...
  // ── Totals ────────────────────────────────────────────────────────
  const TOTALS_NEED = 4 + 14 * 4 + 10 + 30;
  if (y - TOTALS_NEED < MARGIN) newPage();
  ...
  function totRow(label: string, value: string, highlight = false): void {
    if (highlight) fillRect(TOT_X, y - 14, LAB_W + VAL_W, 14, C_BLUE);
    const textColor: RgbColor = highlight ? C_WHITE : C_DARK;
    drawText(label, TOT_X + 5, y - 10, { size: 8.5, bold: highlight, color: textColor });
    drawText(value, TOT_X + LAB_W, y - 10,
      { size: 8.5, bold: highlight, color: textColor, align: "right", maxWidth: VAL_W - 5 });
